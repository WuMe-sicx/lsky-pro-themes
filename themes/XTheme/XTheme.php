...
class XTheme extends ThemeAbstract
{
    ...
    {
        Route::get('/{any}', fn (): View => view("{$this->id}::index"))->where('any', '^(?!api).*');
    }
     ...
     * 创建广告区块表单字段
     */
    protected function createAdsSectionFormField(string $section, string $title, string $description): Section
    {
     ...
     * 创建单侧广告字段组
     */
    protected function createSideAdFieldset(string $basePath, string $side, string $label): Fieldset
    {
        $path = "{$basePath}.{$side}";
        $typePath = "{$path}.type";

        return Fieldset::make($label)->schema([
            Checkbox::make("{$path}.enabled")
                ->label('启用'),
     ...
     * 核心能力模块广告
     */
    protected function getSiteAdsFeaturesFormField(): Section
    {
     ...
     * 应用场景模块广告
     */
    protected function getSiteAdsScenariosFormField(): Section
    {
     ...
     * 数据统计模块广告
     */
    protected function getSiteAdsStatsFormField(): Section
    {
     ...
     * 常见问题模块广告
     */
    protected function getSiteAdsFaqFormField(): Section
    {
     ...
     * 版本信息展示区块
     */
    protected function getVersionInfoSection(): Section
    {
        ...
        return Section::make('主题信息')
            ->description($description)
            ->collapsed()
            ->schema([
                Placeholder::make('version_info')
                    ->label('版本信息')
                    ->content(fn () => new HtmlString($this->buildVersionInfoHtml($updateInfo, $hasUpdate))),
            ]);
    }
    ...
    protected function buildVersionInfoHtml(?array $updateInfo, bool $hasUpdate): string
    {
        ...
        $html .= '<p><strong>当前版本:</strong> v' . e($this->version) . '</p>';
        $html .= '<p><strong>作者:</strong> ' . e($this->author) . '</p>';
        $html .= '<p><strong>官网:</strong> <a href="' . e($this->url) . '" target="_blank" class="text-primary-500 hover:underline">' . e($this->url) . '</a></p>';
        ...
        if ($hasUpdate && $updateInfo) {
            ...
            $html .= '<p class="font-semibold text-warning-700 dark:text-warning-400">🎉 发现新版本: v' . e($updateInfo['version']) . '</p>';
            ...
            if (!empty($updateInfo['download_url'])) {
                $html .= '<p class="mt-2"><a href="' . e($updateInfo['download_url']) . '" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-warning-500 hover:bg-warning-600 text-white text-sm font-medium rounded-lg transition-colors">前往下载</a></p>';
            }
            $html .= '</div>';
        } elseif ($updateInfo) {
    ...
    protected function checkForUpdates(): void
    {
        ...
        // 每个会话只提醒一次，避免重复弹窗
        $sessionKey = "theme_{$this->id}_update_notified_{$this->version}";
    ...
    protected function getUpdateInfo(): ?array
    {
        ...
        $cacheKey = "theme_{$this->id}_update_info";
        ...
        try {
            $response = Http::timeout(3)->get($this->updateCheckUrl);
            ...
            Log::debug("XTheme update check failed: {$e->getMessage()}");
        }
    ...
    {
        Cache::forget("theme_{$this->id}_update_info");
    }
}